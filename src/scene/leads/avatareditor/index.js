...
class AvatarEdit extends React.Component {
	...
	onChangeImage = (e) => {
		...
		$("#btn_save").on("click", function(){
			$uploadCrop.croppie('result', 'html').then(function (resp) {
				self.setState({cropped_img: resp.children[0].src});
				self.state.onGetCrop(resp.children[0].src);
	...
	render () {
		...
		let pro_img = this.state.user.profile_image || this.state.cropped_img || profile_img;
