...
const mapDispatchToProps = (dispatch) => {
    return ({
        getProperty: (mlsId, param) => { getProperty(mlsId, param, dispatch) },
    });
}
...
const mapStateToProps = (state) => {
    ...
    return ({ 
        property: state.property.selected_property,
        properties: state.property.searchResults
    });
}


class Propert extends React.Component {
	constructor (props) {
		super(props);
		this.state = {
			mlsId: ""
	...
	componentWillMount() {
		...
		this.setState({ mlsId: id });
		let param = "c2ltcGx5cmV0czpzaW1wbHlyZXRz";
		this.props.getProperty(id, param);
	}
...
export default connect(mapStateToProps, mapDispatchToProps)(Propert);